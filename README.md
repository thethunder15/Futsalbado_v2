...
- **Autenticação Simples**: Os usuários conseguem criar sua conta de jogador usando nome, celular (WhatsApp), posição (agora incluindo `Lateral`), peso em KG e avaliação (`Perna de Pau` à `Seleção`).
...
Para evitar as clássicas brigas nos famosos "par ou ímpar" ou "tirar o time", os administradores possuem um botão especial **"Sorteia Ai"**. Ele envia toda a lista de confirmados (com as respectivas posições, peso em kg e nível de habilidade) para uma API inteligente do *Google Gemini*, que processa instantaneamente cruzamentos numéricos e devolve duas equipes perfeitamente equilibradas. A IA agora considera o balanço físico distribuindo jogadores mais pesados igualmente entre os times, garantindo uma partida super disputada e justificando o porquê de cada divisão.
